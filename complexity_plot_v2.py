'''
using median value to deal with the same number of assets
'''
import numpy as np
...
def process_data_kappa_midian(list_x):
    ...
    for row in list_x:
        ...
        if last_num != row[0] and count != 0:
            num_assets.append(row[0])
            kappa_opt_median.append(np.median(np.array(kappa_opt_t)))
            kappa_origin_median.append(np.median(np.array(kappa_origin_t)))
            count = 0
            kappa_opt_t = []
            kappa_origin_t = []
...
def plot_kappa(num_assets, kappa_opt_median, kappa_origin_median):
    # print(kappa_opt_median)
    # print(kappa_origin_median)
    plt.plot(num_assets, kappa_opt_median, 'b*-', alpha=0.5,
             linewidth=1, label='$\kappa$ of SAPO')  # 'bo-'表示蓝色实线，数据点实心原点标注
    plt.plot(num_assets, kappa_origin_median, 'r*-', alpha=0.5,
             linewidth=1, label='$\kappa$ without optimization')  # 'bo-'表示蓝色实线，数据点实心原点标注
    # plot中参数的含义分别是横轴值，纵轴值，线的形状（'s'方块,'o'实心圆点，'*'五角星   ...，颜色，透明度,线的宽度和标签 ，

    polynomial_opt = np.polyfit(num_assets, kappa_opt_median, 5)
    polynomial_orig = np.polyfit(num_assets, kappa_origin_median, 5)
...
def polynomial_fitting(num_assets, kappa_opt_median, kappa_origin_median):
    polynomial_opt = np.polyfit(num_assets, kappa_opt_median, 5)
    polynomial_orig = np.polyfit(num_assets, kappa_origin_median, 5)
    print("polynomial_opt polynomial coefficients:", polynomial_opt)
    print("polynomial_orig polynomial coefficients:", polynomial_orig)
...
def plot_complexity1(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
    ...
    for i in range(len(num_assets)):
        time_hhl.append(np.log(int(2 ** np.ceil(np.log2(num_assets[i]+2)))) *
                        kappa_origin_median[i]**2*0.25*1/epsilon)
        time_sapo.append(np.log(int(2 ** np.ceil(np.log2(num_assets[i]+2))))
                         * kappa_opt_median[i]**2*0.25*1/epsilon)
...
def plot_complexity6(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
...
def plot_complexity4(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    """
...
def plot_complexity2(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
    ...
    for i in range(len(num_assets)):
        ...
        time_gc.append(num*kappa_origin_median[i]*np.log(1/epsilon))
        time_hhl.append(np.log(
            int(2 ** np.ceil(np.log2(num+2))))*kappa_origin_median[i]**2*0.25*1/epsilon)
        time_sapo.append(np.log(
            int(2 ** np.ceil(np.log2(num+2))))*kappa_opt_median[i]**2*0.25*1/epsilon)
...
def plot_complexity5(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
...
def plot_complexity3(num_assets, kappa_opt_median, kappa_origin_median, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
...
if __name__ == "__main__":
    data = read_data()
    num_assets, kappa_opt_median, kappa_origin_median = process_data_kappa_midian(
        data)
    p_opt, p_orig = polynomial_fitting(
        num_assets, kappa_opt_median, kappa_origin_median)
    num_assets_large = [1000, 5000, 10000, 30000,
                        60000, 100000, 200000, 400000, 600000]
    if sys.argv[1] == "1":
        plot_kappa(num_assets, kappa_opt_median, kappa_origin_median)
    elif sys.argv[1] == "2":
        plot_complexity1(num_assets, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
    elif sys.argv[1] == "3":
        plot_complexity2(num_assets, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
    elif sys.argv[1] == "4":
        plot_complexity3(num_assets, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
    elif sys.argv[1] == "5":
        plot_complexity4(num_assets_large, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
    elif sys.argv[1] == "6":
        plot_complexity5(num_assets, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
    elif sys.argv[1] == "7":
        plot_complexity6(num_assets, kappa_opt_median,
                         kappa_origin_median, p_opt, p_orig)
