...
# for num in nums:
#     print(num, p_orig(num), "kappa without optimization")
#     print(num, p_opt(num), "kappa with SAPO")
...
def plot_complexity1(num_assets, kappa_opt_mean, kappa_origin_mean, p_opt, p_orig):
    ...
    for i in range(len(num_assets)):
        time_hhl.append(np.log(num_assets[i]) *
                        kappa_origin_mean[i]**2*0.25*1/epsilon)
        time_sapo.append(np.log(num_assets[i])
    ...
    #     time_hhl.append(np.log(num)*p_orig(num)**2*0.25*1/epsilon)
    #     time_sapo.append(np.log(num)*p_opt(num)**2*0.25*1/epsilon)
...
def plot_complexity6(num_assets, kappa_opt_mean, kappa_origin_mean, p_opt, p_orig):
    epsilon = 1/8
    time_gauss = []
    time_gc = []
    time_hhl = []
...
def plot_complexity3(num_assets, kappa_opt_mean, kappa_origin_mean, p_opt, p_orig):
    ...
    for num in nums:
        ...
        time_sapo.append(np.log(num)*0.25*1/epsilon)
    time_gauss = np.log(time_gauss)
    time_gc = np.log(time_gc)
    time_sapo = np.log(time_sapo)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "1":
        ...
        plot_complexity6(num_assets, kappa_opt_mean,
                         kappa_origin_mean, p_opt, p_orig)
